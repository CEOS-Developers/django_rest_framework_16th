...
class TodoClassAPI(APIView):
    def get(self, request, id=id):
        todo_class = get_object_or_404(TodoClass, id=id)
        if (todo_class.deleted_flag):  # check if the class had been soft-deleted
            return Response("Todo Class does not exist", status=status.HTTP_404_NOT_FOUND)
        serializer = TodoClassSerializer(todo_class)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def patch(self, request, id=id):
        ...
        if (request.data != {}):
            ...
            todo_class.deleted_flag = True  # soft-delete todo_class
        todo_class.save()
        serializer = TodoClassSerializer(todo_class)
        return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
